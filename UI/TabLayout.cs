	...
	public class TabLayout : MonoBehaviour
	{
			...
			: base(value)
		{
			...
			else
			{
				...
				foreach (var idk in resources)
				{
					if (idk.obj.name.Equals("Page_Tab_Backdrop") && idk.obj.GetIl2CppType() == UnhollowerRuntimeLib.Il2CppType.Of<Sprite>())
					{
						sprite = idk.obj.Cast<Sprite>();
					}
				}
				if (sprite != null)
				{
		...
		public void ApplySorting(bool applyDefault = false)
		{
			...
			for (int i = 0; i < base.transform.childCount; i++)
			{
				...
				if (transform.gameObject.name != "Background_QM_PagePanel")
				{
					...
					{
						UIPage uIPage;
						menuStateController.field_Private_Dictionary_2_String_UIPage_0.TryGetValue(text, out uIPage);
